...
<div class="flex flex-col gap-2">
	...
	{#each locations as location, idx (location.id)}
		<div class="border-base-300 overflow-hidden rounded-lg border">
			<!-- Header -->
			<div class="flex w-full items-center justify-between px-3 py-2 transition-colors {expandedId ===
			location.id
				? 'bg-secondary/20'
				: 'hover:bg-base-200'}">
				<button
				...
				>
					<span class="text-sm font-medium truncate">{location.name || 'Unnamed'}</span>
				</button>
	...
	{#if locations.length === 0}
		<p class="text-base-content/40 text-center text-xs">No locations. Add one above.</p>
	{/if}
</div>
