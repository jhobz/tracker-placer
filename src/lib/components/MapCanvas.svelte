<script lang="ts">
	...
	function renderedToImage(rx: number, ry: number) {
		if (!renderedRect.width || !renderedRect.height) return { x: 0, y: 0 };
		return {
	...
	function handleCanvasClick(e: MouseEvent) {
		if (!appState.placingMode || !map || !containerEl) return;
		const cr = containerEl.getBoundingClientRect();
...
>
		...
		{#if map && naturalWidth > 0}
			...
			{#each map.locationBoxes as box (box.id)}
				...
				<button
					...
					style="left: {renderedRect.left + pos.x}px; top: {renderedRect.top + pos.y}px; width: {rw}px; height: {rh}px;"
	...
	{:else}
		<div class="flex h-full flex-col items-center justify-center gap-4">
			<svg
				class="text-base-content/20 h-20 w-20"
				fill="none"
				stroke="currentColor"
				viewBox="0 0 24 24"
			>
